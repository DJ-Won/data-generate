...
def sample_orientations(
    ...
    yaw_edges = np.linspace(
        yaw_center + match.yaw_search_range_deg[0],
        yaw_center + match.yaw_search_range_deg[1],
        count + 1,
    )
    yaws = rng.uniform(yaw_edges[:-1], yaw_edges[1:])
    rng.shuffle(yaws)
    pitches = rng.uniform(
        pitch_center + match.pitch_search_range_deg[0],
        pitch_center + match.pitch_search_range_deg[1],
        count,
    )
    if cfg.scene.scene_type == "object":
        pitches = np.clip(pitches, -88.9, 88.9)
    rolls = rng.uniform(
        seed.roll_deg + match.roll_search_range_deg[0],
        seed.roll_deg + match.roll_search_range_deg[1],
        count,
    )
        ...
        "image_quality": (
            {
                ...
                "accepted": topiq_nr_score > cfg.initialization.traversal.random.topiq_nr_threshold_l,
            }
...
def run_traversal(cfg: TraversalConfig) -> dict:
    ...
    plan_json = {
        ...
        "object_sampling": (
            ...
            if cfg.scene.scene_type == "object" else None
        ),
        "positions": [
            {
                "index": item.index,
